...
# from networks import ModelBuilder, SegmentationModule
...
class Trainer:
    def __init__(self, options):
        ...
        self.models["depth"] = networks.NQGN_SparseDecoder()
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                # mask warping
                # outputs[("proj_mask", frame_id, scale)] = F.grid_sample(
                #     outputs[("quad_mask", scale)],
                #     outputs[("sample", frame_id, scale)],
                #     padding_mode="border")
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                rep_loss = self.compute_reprojection_loss(pred, target)
                reprojection_losses.append(rep_loss)

            reprojection_losses = torch.cat(reprojection_losses, 1)
            # quad_masking_losses = torch.cat(quad_masking_losses, 1)
    ...
    def compute_mask_quadtree(self, disp, max_depth_level, criterion):
        ...
        for i in range(max_depth_level, 0, -1):
            ...
            mask[i] = (torch.max(a, axis=1).values - torch.min(a, axis=1).values) > criterion
            mask[i] = torch.unsqueeze(mask[i],1).type(torch.FloatTensor).to(self.device)
